...
$lien_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);

if (!$lien_id) {
    // Gérer l'erreur, peut-être rediriger avec un message
    $_SESSION['delete_message'] = '<p style="color: red;">ID de lien invalide ou manquant pour la suppression.</p>';
    header('Location: ../index.php?page=liens_list');
    exit();
}
...
// Suppression
try {
    $stmt = $pdo->prepare("DELETE FROM liens WHERE ID = :id");
    $stmt->bindParam(':id', $lien_id, PDO::PARAM_INT);
    
    if ($stmt->execute()) {
        if ($stmt->rowCount() > 0) {
            $_SESSION['delete_message'] = '<p style="color: green;">Lien supprimé avec succès !</p>';
        } else {
            $_SESSION['delete_message'] = '<p style="color: orange;">Aucun lien trouvé avec cet ID pour la suppression.</p>';
        }
    } else {
        $_SESSION['delete_message'] = '<p style="color: red;">Erreur lors de la suppression du lien.</p>';
    }
} catch (PDOException $e) {
